...
describe('prompt.builder', () => {
  it('buildPrompt throws on empty question', () => {
    expect(() => buildPrompt('', ['ctx'])).toThrow('Question cannot be empty');
  });

  it('buildPrompt throws on empty context', () => {
    expect(() => buildPrompt('q', [])).toThrow('At least one context chunk is required');
  });

  it('buildPrompt returns formatted prompt containing fragment markers', () => {
  ...
  it('buildSimplePrompt returns concise prompt', () => {
    const p = buildSimplePrompt('Q', ['a', 'b']);
    expect(p).toContain('Contexto:');
    expect(p).toContain('Pregunta: Q');
  });

  it('buildConversationalPrompt includes history when provided', () => {
  ...
  it('buildSummarizationPrompt throws on empty topic', () => {
    expect(() => buildSummarizationPrompt('', ['x'])).toThrow('Topic cannot be empty');
  });

  it('estimateTokens approximates tokens', () => {
    expect(estimateTokens('abcd')).toBeGreaterThanOrEqual(1);
    expect(estimateTokens('')).toBe(0);
  });

  it('truncateContext shortens context to maxTokens and adds ellipsis when partial', () => {
